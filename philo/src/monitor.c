...
/*   Updated: 2023/02/04 17:24:42 by troberts         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	stop_simulation(t_philo *philo)
{
	...
	if (philo->time_of_last_meal != -1 && get_time_since_start(philo->common) - philo->time_of_last_meal >= philo->common.time.time_to_die)
		return_code = DEAD_PHILO;
	pthread_mutex_unlock(&philo->update_status);
	return (return_code);
}

pthread_t	launch_monitor(t_philo *philo_struct, t_common common, t_monitor *monitor_args)
{
		...
		while (i < monitor->common.nbr_philosophers)
		{
			...
			if (stop_simulation_return == DEAD_PHILO)
			{
				...
				*(monitor->common.is_dead) = 1;
				...
				// printf("%i, %i, %i\n", get_time_since_start(monitor->common), philo_struct[i].time_of_last_meal, monitor->common.time.time_to_die);
			...
			if (stop_simulation_return == NO_MORE_MEALS)
			{
				...
				if (monitor->common.nbr_philo_meals_finished == monitor->common.nbr_philosophers)
					return (NULL);
			}
